...
### HW-10. Spring MVC REST
Веб-приложение каталога книг переработано в соответствии с архитектурным стилем REST. Реализация выполнена на базе предыдущей работы (HW-09) с переходом от server-side рендеринга Thymeleaf к клиент-серверной архитектуре с использованием JavaScript и REST API.

### Основные изменения и особенности реализации:
**Архитектурные изменения:**
- Удалены Thymeleaf шаблоны и server-side рендеринг
- Реализованы REST контроллеры (@RestController) для всех сущностей
- Клиентская часть переписана на чистом JavaScript с использованием Fetch API
- Данные передаются в формате JSON между клиентом и сервером

**Реализованные REST endpoints (соответствуют ресурсному стилю):**
**Книги (Books):**
...
- GET /comments/book/{bookId} - получение комментариев по ID книги
- POST /comments - создание нового комментария
- DELETE /comments/{id} - удаление комментария

**Особенности REST реализации:**
- URL эндпойнтов соответствуют ресурсному стилю (отсутствуют глаголы в URL)
- Действия над сущностями выражены исключительно через HTTP-методы
- Используются соответствующие HTTP статус-коды (200, 201, 204, 404)
- Данные передаются в формате DTO (Data Transfer Object)
- Реализована обработка ошибок через ResponseEntity

**Клиентская часть:**
- Single Page Application (SPA) на чистом JavaScript
- Динамическое обновление контента без перезагрузки страницы
- Использование Fetch API для взаимодействия с REST endpoints
...
- Протестированы все CRUD endpoints с мокированием сервисов
- Проверены сценарии успешного выполнения операций и обработки ошибок
- Протестированы HTTP статус-коды и структура JSON ответов